...
typedef struct listnode {
	char data[100];    // largest size of input data
...
int main() {
	char filename[50];
	printf("Input filename(test.txt):");
	scanf("%s", filename);
	...
	textarray = (listpointer)malloc(sizeof(listnode) * (num_counter+1)); //申請(num_counter+1)個節點
	for (i = 0; i < (num_counter + 1); i++) {                              // Reset array
		(textarray + i)->digit = 0;
	...
	//================================================================
	printf("\nRemove duplicate integers for each list:\n");
	int upper = 1;
	for (i = 0; i < num_counter + 1; i++) {
